...
/*int ft_solve_xxx(t_dict *dict, char *num, int n_zero, t_solve *sol))
{
	char	tmp[MAX_LONG_KEY];

	if (!ft_solve(dict, ft_itoa(v_num / 100, tmp), 0, sol))
		return (0)
	if (find_value(dict, ft_itoa(100, tmp), (*sol).words[(*sol).size++]) == NULL)
		return (0);
	return (ft_solve(dict, ft_itoa(v_num % 100, tmp), 0, sol));
}*/
...
int	ft_solve_less100(t_dict *dict, int v_num, t_solve *sol)
{
	...
	if (v_num / 20 == 0)
	{
		if (find_value(dict, ft_itoa(v_num, tmp), (*sol).words[(*sol).size++]) == NULL)
			return (0);
	}
	else
	{
		if (find_value(dict,
				ft_itoa((v_num / 10) * 10, tmp), (*sol).words[(*sol).size++]) == NULL)
			return (0);
		if (v_num % 10 != 0)
		{
			if (find_value(dict, ft_itoa(v_num % 10, tmp), (*sol).words[(*sol).size++]) == NULL)
				return (0);
			}
	}
	return (1);
...
int	ft_resolve_number(t_dict *dict, char *num, t_solve *sol)
{
	...
	if (ft_atoi(num) == 0)
		find_value(dict, tmp, (*sol).words[0]);
	else
	{
